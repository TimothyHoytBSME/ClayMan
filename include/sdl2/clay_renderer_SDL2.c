...
static void Clay_SDL2_Render(SDL_Renderer *renderer, Clay_RenderCommandArray renderCommands, SDL2_Font *fonts)
{
    for (uint32_t i = 0; i < renderCommands.length; i++)
    {
        ...
        switch (renderCommand->commandType)
        {
            ...
            case CLAY_RENDER_COMMAND_TYPE_IMAGE: { //todo
            ...
            case CLAY_RENDER_COMMAND_TYPE_BORDER: {
                ...
                if (config->width.left > 0) {
                    ...
                    SDL_FRect rect = { boundingBox.x, boundingBox.y + config->cornerRadius.topLeft, (float)config->width.left, (float)boundingBox.height - (float)config->cornerRadius.topLeft - (float)config->cornerRadius.bottomLeft };
                    SDL_RenderFillRectF(renderer, &rect);
                }

                if (config->width.right > 0) {
                    SDL_SetRenderDrawColor(renderer, CLAY_COLOR_TO_SDL_COLOR_ARGS(config->color));
                    SDL_FRect rect = { boundingBox.x + boundingBox.width - config->width.right, boundingBox.y + config->cornerRadius.topRight, (float)config->width.right, (float)boundingBox.height - (float)config->cornerRadius.topRight - (float)config->cornerRadius.bottomRight };
                    SDL_RenderFillRectF(renderer, &rect);
                }
                ...
                if (config->width.top > 0) {
                    SDL_SetRenderDrawColor(renderer, CLAY_COLOR_TO_SDL_COLOR_ARGS(config->color));
                    SDL_FRect rect = { boundingBox.x + config->cornerRadius.topLeft, boundingBox.y, boundingBox.width - config->cornerRadius.topLeft - config->cornerRadius.topRight, (float)config->width.top };
                    SDL_RenderFillRectF(renderer, &rect);
                }

                if (config->width.bottom > 0) {
                    SDL_SetRenderDrawColor(renderer, CLAY_COLOR_TO_SDL_COLOR_ARGS(config->color));
                    SDL_FRect rect = { boundingBox.x + config->cornerRadius.bottomLeft, boundingBox.y + boundingBox.height - config->width.bottom, boundingBox.width - config->cornerRadius.bottomLeft - config->cornerRadius.bottomRight, (float)config->width.bottom };
                    SDL_RenderFillRectF(renderer, &rect);
                }
